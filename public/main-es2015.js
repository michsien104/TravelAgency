...
let listFilterPipe = class listFilterPipe {
    transform(trips, name, country, since, untill) {
        if (!trips) {
            return trips;
        }
        let result = trips;
...
let AuthGuardService = class AuthGuardService {
    ...
    canActivate(next, state) {
        return this.authService.authState$
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(state => {
            if (state !== null) {
                return true;
            }
...
let MongoTripRepoService = class MongoTripRepoService {
    constructor(http) {
        this.http = http;
...
MongoTripRepoService.ctorParameters = () => [
...
let ShoppingRepositoryService = class ShoppingRepositoryService {
...
let CommentsBoxComponent = class CommentsBoxComponent {
    constructor() {
...
let NewTripsComponent = class NewTripsComponent {
    constructor(formBuilder, tripRepository) {
        this.formBuilder = formBuilder;
    ...
    ngOnInit() {
        ...
        this.form.valueChanges.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["debounceTime"])(1000)).subscribe((value) => {
    ...
    onSubmit() {
        if (this.form.valid) {
            let trip = new _models_trip_model__WEBPACK_IMPORTED_MODULE_3__["Trip"](this.form.value);
            let body = JSON.stringify(trip);
            this.tripRepository.addTrip(body).subscribe((response) => {
...
NewTripsComponent.ctorParameters = () => [
    { type: _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormBuilder"] },
...
let ShoppingCartComponent = class ShoppingCartComponent {
    constructor(shoppingCartRepository, tripsService, router) {
...
let SingleTripComponent = class SingleTripComponent {
    constructor(shoppingCartRepository, tripRepository) {
        this.shoppingCartRepository = shoppingCartRepository;
        this.tripRepository = tripRepository;
...
SingleTripComponent.ctorParameters = () => [
    { type: _services_nodeJSData_shopping_repository_service__WEBPACK_IMPORTED_MODULE_2__["ShoppingRepositoryService"] },
    { type: _services_nodeJSData_mongo_trip_repo_service__WEBPACK_IMPORTED_MODULE_3__["MongoTripRepoService"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
...
let TripDetailsComponent = class TripDetailsComponent {
    constructor(route, tripRepository, config) {
        this.route = route;
        this.tripRepository = tripRepository;
    ...
    getTrip() {
        this.route.paramMap.subscribe(params => {
            this.tripRepository.getTrip(params.get('id')).subscribe((response) => {
...
let TripsComponent = class TripsComponent {
    constructor(tripRepository, router) {
        this.tripRepository = tripRepository;
        this.router = router;
    ...
    deleteTrip(trip) {
        this.tripRepository.deleteTrip(trip._id).subscribe((response) => {
            if (response.success == true) {
